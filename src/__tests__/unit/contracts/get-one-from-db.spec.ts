import { GetOneFromDb } from '@/contracts'
import { collectionNameMock, mockCreated } from '../__mocks__'
import { IDatabaseGetOneSpy } from '../__spys__'

describe('GetOneFromDb', () => {
  function makeSut() {
    ...
    return {
      sut: new GetOneFromDb(),
      collectionName,
      db: new IDatabaseGetOneSpy(expected),
      by,
      matching,
      expected,
    }
  ...
  describe('IDatabaseGetOne', () => {
    it('should be called with right values', async () => {
      ...
      await sut.getOne(collectionName, db, by, matching)
    ...
    it('should sut throw if throws', async () => {
      ...
      await expect(
        sut.getOne(collectionName, db, by, matching),
    ...
    it('should sut return null if returns null', async () => {
      ...
      const result = await sut.getOne(collectionName, db, by, matching)
